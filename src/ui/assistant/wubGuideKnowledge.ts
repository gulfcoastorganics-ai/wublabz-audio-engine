...
const KNOWLEDGE: KnowledgeEntry[] = [
  ...
  {
    id: 'normalize-nondestructive',
    title: 'Does Normalize Change Your File?',
    body: 'No. Normalize in WubLabz Studio is non-destructive. Your original audio file is never modified. The gain adjustment is stored as clip metadata and applied in real time during playback and export. Resetting edits removes it instantly.',
    keywords: ['does normalize change my file', 'normalize file', 'normalize permanent', 'normalize nondestructive', 'normalize destructive'],
    highlightTarget: 'clip',
    actions: [{ type: 'focusArrangement' }],
    steps: [
      'Right-click a clip and choose "Normalize Clip", or toggle Normalize in the Clip Inspector.',
      'The NRM badge shows the edit is active.',
      'Your original audio file is unchanged.',
      'Right-click and choose "Reset Edits" to remove the normalization at any time.',
    ],
  },
  {
    id: 'normalize',
  ...
  {
    id: 'reverse-mechanics',
    title: 'How Reverse Works',
    body: 'Reversing a clip plays the audio back-to-front. The waveform is read in reverse order during playback and export. It does not permanently alter your audio file — toggling Reverse off restores normal direction instantly.',
    keywords: ['what does reverse do', 'how does reverse work', 'reverse mechanics', 'reverse playback', 'reverse effect'],
    highlightTarget: 'clip',
    actions: [{ type: 'focusArrangement' }],
    steps: [
      'Select an audio clip in the arrangement.',
      'Toggle "Reverse" in the Clip Inspector, or right-click and choose "Reverse Clip".',
      'The REV badge confirms playback is reversed.',
      'Toggle off to instantly restore normal direction.',
    ],
  },
  {
    id: 'reverse-audio',
  ...
  },
  {
    ...
    body: 'Destructive editing permanently changes the audio data on disk. Non-destructive editing stores parameters separately and applies them in real time. WubLabz Studio uses non-destructive editing for all clip edits — gain, reverse, fades, and normalize — so your original files are always safe.',
    keywords: ['destructive editing', 'what is destructive', 'destructive vs nondestructive', 'non destructive', 'original file safe'],
    highlightTarget: 'clip',
    actions: [{ type: 'focusArrangement' }],
    steps: [
      'All clip edits in WubLabz are non-destructive.',
      'Gain, reverse, fades, and normalize are stored as metadata.',
      'Your original audio files are never modified.',
      'You can reset all edits on any clip at any time.',
    ],
  },
  {
    id: 'edits-permanent',
    title: 'Are My Clip Edits Permanent?',
    body: 'No. All clip edits (gain, reverse, fades, normalize) are non-destructive and stored with the project. You can change or remove them at any time. Right-click a clip and choose "Reset Edits" to remove all edits from that clip instantly.',
    keywords: ['are my edits permanent', 'are edits permanent', 'are my edits saved', 'undo edits', 'permanent changes', 'reset clip edits', 'remove edits'],
    highlightTarget: 'clip',
    actions: [{ type: 'focusArrangement' }],
    steps: [
      'Clip edits are saved with your project.',
      'They are never baked into your original audio file.',
      'Right-click a clip and choose "Reset Edits" to clear all edits.',
      'The clip immediately plays as originally imported.',
    ],
  },
